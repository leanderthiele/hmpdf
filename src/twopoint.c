...
{//{{{
    ...
    d->tp->dtsq = (double ***)malloc(d->n->gr->Nz * sizeof(double **));
    d->tp->t = (double ***)malloc(d->n->gr->Nz * sizeof(double **));
    d->tp->ac = (complex **)malloc(d->n->gr->Nz * sizeof(complex *));
    d->tp->au = (complex *)fftw_malloc((d->n->gr->Nsignal/2+1) * sizeof(complex));
    double *au_real = (double *)(d->tp->au);

    double *tempc_real = (double *)fftw_malloc((d->n->gr->Nsignal+2)*sizeof(double));
    complex *tempc_comp = (complex *)tempc_real;
    fftw_plan plan_c = fftw_plan_dft_r2c_1d(d->n->gr->Nsignal, tempc_real, tempc_comp, FFTW_MEASURE);
    fftw_plan plan_u = fftw_plan_dft_r2c_1d(d->n->gr->Nsignal, au_real, d->tp->au, FFTW_MEASURE);
    ...
    {
        d->tp->dtsq[z_index] = (double **)malloc(d->n->gr->NM * sizeof(double *));
        d->tp->t[z_index] = (double **)malloc(d->n->gr->NM * sizeof(double *));
        d->tp->ac[z_index] = (complex *)malloc((d->n->gr->Nsignal/2+1) * sizeof(complex));
        // null the FFT array
        for (int ii=0; ii<d->n->gr->Nsignal+2; ii++)
        {
            tempc_real[ii] = 0.0;
        }
        ...
        {
            d->tp->dtsq[z_index][M_index] = (double *)malloc(d->n->gr->Nsignal * sizeof(double));
            d->tp->t[z_index][M_index] = (double *)malloc(d->n->gr->Nsignal * sizeof(double));
            ...
            for (int ii=0; ii<d->n->gr->Nsignal; ii++)
            {
                au_real[ii] -= M_PI * n * d->tp->dtsq[z_index][M_index][ii]
                               * d->n->gr->Mweights[M_index] * d->n->gr->zweights[z_index]
    ...
    for (int ii=d->n->gr->Nsignal/2; ii>=0; ii--)
    {
...
{//{{{
    // null tempc
    for (int ii=0; ii<d->n->gr->Nsignal*(d->n->gr->Nsignal+2); ii++)
    {
    ...
    for (int M_index=0; M_index<d->n->gr->NM; M_index++)
    {
        ...
        // loop over the direction that is Nsignal long
        {
            ...
            // loop over the direction that is Nsignal+2 long
            {
                ...
                ws->tempc_real[ii*(d->n->gr->Nsignal+2)+jj] += temp * b;
                ws->pdf_real[ii*(d->n->gr->Nsignal+2)+jj] += temp
                                                             * gsl_pow_2(d->c->comoving[z_index])
                                                             / d->c->hubble[z_index]
                                                             * d->n->gr->zweights[z_index];
            }
        }
        ...
        for (int ii=0; ii<d->n->gr->Nsignal; ii++)
        {
            ws->pdf_real[ii*(d->n->gr->Nsignal+2)]
                -=  d->tp->dtsq[z_index][M_index][ii]
                    * M_PI * n
                    * gsl_pow_2(d->c->comoving[z_index]) / d->c->hubble[z_index]
                    * d->n->gr->Mweights[M_index] * d->n->gr->zweights[z_index]
...
{//{{{
    complex a1 = redundant(d->n->gr->Nsignal, d->tp->ac[z_index], i1);
    complex a2 = d->tp->ac[z_index][i2];
    complex b = b12[i1*(d->n->gr->Nsignal/2+1)+i2]
                - b12[i1*(d->n->gr->Nsignal/2+1)]
                - b12[i2] + b12[0];
...
{//{{{
    ...
    for (int ii=0; ii<d->n->gr->Nsignal*(d->n->gr->Nsignal/2+1); ii++)
    {
        ws->bc[ii] = 0.0;
    }
    ...
    for (int z_index=0; z_index<d->n->gr->Nz; z_index++)
    {
        ...
        for (int ii=0; ii<d->n->gr->Nsignal; ii++)
        {
            for (int jj=0; jj<ii; jj++)
            {
                ws->tempc_real[jj*(d->n->gr->Nsignal+2)+ii]
                    = ws->tempc_real[ii*(d->n->gr->Nsignal+2)+jj];
            }
        }
        ...
        {
            for (int jj=0; jj<d->n->gr->Nsignal/2+1; jj++)
            // loop over the short direction
            {
                ...
                ws->bc[ii*(d->n->gr->Nsignal/2+1)+jj] += temp * d->n->gr->zweights[z_index];
            }
        }
    }
}//}}}
...
{//{{{
    ...
    for (int ii=0; ii<d->n->gr->Nsignal; ii++)
    {
        for (int jj=0; jj<ii; jj++)
        {
            ws->pdf_real[jj*(d->n->gr->Nsignal+2)+ii]
                = ws->pdf_real[ii*(d->n->gr->Nsignal+2)+jj];
        }
    }
    ...
    for (int ii=d->n->gr->Nsignal-1; ii>=0; ii--)
    {
        for (int jj=d->n->gr->Nsignal/2; jj>=0; jj--)
        {
            ws->pdf_comp[ii*(d->n->gr->Nsignal/2+1)+jj]
                = cexp(ws->pdf_comp[ii*(d->n->gr->Nsignal/2+1)+jj]
                       - ws->pdf_comp[ii*(d->n->gr->Nsignal/2+1)]
                       - ws->pdf_comp[jj] + ws->pdf_comp[0]
                       + d->tp->au[jj] + redundant(d->n->gr->Nsignal, d->tp->au, ii)
                       + ws->bc[ii*(d->n->gr->Nsignal/2+1)+jj])
                  / gsl_pow_2((double)(d->n->gr->Nsignal));
        }
    }
    ...
    {
        regularize(d->n->gr->Nsignal, ws->pdf_comp);
    }
